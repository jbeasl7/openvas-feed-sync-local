...
if(description)
{
  ...
  script_name("IlohaMail < 0.8.11 User Parameter Vulnerability");
  ...
  script_tag(name:"summary", value:"IlohaMail does not properly sanitize the 'user' parameter, which
  could allow a remote attacker to execute arbitrary code either on the target or in a victim's
  browser when a victim views a specially crafted message with an embedded image as long as PHP's
  magic quotes setting is turned off (it's on by default) and the MySQL backend is in use.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"affected", value:"IlohaMail version 0.8.10 and prior.");

  script_tag(name:"solution", value:"Update to version 0.8.11 or later.");
