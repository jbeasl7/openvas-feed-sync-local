...
if(description)
{
  ...
  script_name("Possible Trojan Horse Detection (Open Port Based)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2002 Michel Arboi");
  script_family("Malware");
  # nb: unknown_services.nasl pulls in all existing service detection VTs so that these services are
  # getting detected previously and not reported below.
  ...
  script_tag(name:"summary", value:"Look for potential trojan horses based on open TCP ports.");

  script_tag(name:"vuldetect", value:"Checks all during the portscan phase found / open TCP ports
  against an internal list of potential trojan horses and reports them. If the port(s) in question
  are providing / reporting a service banner it will be reported as well.");

  script_tag(name:"insight", value:"An unknown service runs on this port. It is sometimes opened by
  Trojan horses.
...
#
trojanlist = "UDP 1 Socket de Troie
TCP 2 Death
TCP 8 Nugache
TCP 15 B2
...
if(banner && strlen(banner[1]) > 0) {
  ...
  m = string(m, "\nHere is the service banner identified by method '", banner[0], "':\n\n", hexbanner);
}

security_message(port:port, data:chomp(m));
