...
if(description)
{
  ...
  script_name("LibreOffice Portable Detection (Windows SMB Login)");

  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2018 Greenbone AG");
  script_family("Product detection");
  # nb: Libreoffice dependency was added so we don't detect a registry-based installation twice
  script_dependencies("secpod_libre_office_detect_win.nasl", "lsc_options.nasl");
  ...
  script_tag(name:"summary", value:"SMB login and WMI file search based detection of LibreOffice
  Portable.");
...
fileList = powershell_wmi_file_search_version_query( file_name:"soffice", file_extension:"exe" );
...
foreach filePath( keys( fileList ) ) {
  ...
  if( vers && version = eregmatch( string:vers, pattern:"^([0-9]+\.[0-9]+\.[0-9]+)" ) ) {
    ...
    register_and_report_cpe( app:"Libreoffice Portable", ver:version[1], concluded:vers, base:cpe, expr:"^([0-9.]+)", insloc:location, regPort:0, regService:"smb-login" );
  }
}
