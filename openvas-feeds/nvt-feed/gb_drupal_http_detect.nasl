...
if(description)
{
  ...
  script_tag(name:"summary", value:"HTTP based detection of Drupal.");

  script_xref(name:"URL", value:"https://new.drupal.org/");
...
foreach dir( make_list_unique( "/", "/drupal", "/drupal6", "/drupal7", "/cms", http_cgi_dirs( port:port ) ) ) {
  ...
  if( updaterMatches > 3 ||
    ...
    if( version == "unknown" ) {
      ...
      # /core/themes/claro/js/mobile.install.js?v=10.3.5
      ver = eregmatch( pattern:"js/mobile.install\.js\?v=([0-9.]+)", string:res );
      if( ! isnull( ver[1] ) ) {
        version = ver[1];
        conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
      }
