...
if(description)
{
  ...
  # nb: uTorrent dependency was added so we don't detect a registry-based installation twice
  script_dependencies("gb_utorrent_detect_win.nasl", "lsc_options.nasl");
  ...
  script_tag(name:"summary", value:"SMB login and WMI file search based detection of uTorrent
  Portable.");
...
fileList = powershell_wmi_file_search_version_query( file_name:"utorrent", file_extension:"exe" );
...
foreach filePath( keys( fileList ) ) {
  ...
  if( vers && version = eregmatch( string:vers, pattern:"^([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)" ) ) {
    ...
    # uTorrent is the 32bit version by default.
