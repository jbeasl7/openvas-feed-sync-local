# SPDX-FileCopyrightText: 2005 j_lampe@bellsouth.net
...
if(description)
{
  ...
  script_name("DNS Zone Transfer (AXFR) Test - Active Check");
  ...
  script_dependencies("dns_server_tcp.nasl", "dns_server.nasl",
                      "msdns-server-hostname-disclosure.nasl");
  script_mandatory_keys("dns/server/detected");

  script_xref(name:"URL", value:"https://datatracker.ietf.org/doc/html/rfc5936");
  script_xref(name:"URL", value:"https://datatracker.ietf.org/doc/html/rfc1034");
  script_xref(name:"URL", value:"https://datatracker.ietf.org/doc/html/rfc1035");
  script_xref(name:"URL", value:"https://www.cisa.gov/news-events/alerts/2015/04/13/dns-zone-transfer-axfr-requests-may-leak-domain-information");
  script_xref(name:"URL", value:"https://insights.sei.cmu.edu/library/2002-tech-tip-securing-an-internet-name-server/");

  script_tag(name:"summary", value:"The remote name server allows DNS zone transfers to be
  performed.");

  script_tag(name:"vuldetect", value:"Sends multiple crafted DNS requests and checks the
  responses.");

  script_tag(name:"impact", value:"A zone transfer will allow the remote attacker to instantly
  populate a list of potential targets. In addition, companies often use a naming convention which
  can give hints as to a servers primary application (for instance, proxy.company.com,
  payroll.company.com, b2b.company.com, etc.).

  As such, this information is of great use to an attacker who may use it to gain information about
  the topology of your network and spot new targets.");

  script_tag(name:"solution", value:"Restrict DNS zone transfers to only the servers that absolutely
  need it.");
