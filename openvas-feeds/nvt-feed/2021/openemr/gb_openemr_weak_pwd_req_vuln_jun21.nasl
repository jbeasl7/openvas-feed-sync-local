...
if(description)
{
  ...
  script_tag(name:"summary", value:"OpenEMR is prone to a weak password requirement
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The application does not enforce a maximum password length
  limit during the user creation mechanism. The vulnerability can be reproduced assuming the
  malicious user is aware of the first 72 characters of the victim user's password.");

  script_tag(name:"impact", value:"Successful exploitation may lead to a complete account
  takeover.");
