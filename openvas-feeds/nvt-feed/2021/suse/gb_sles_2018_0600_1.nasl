...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://www.suse.com/support/update/announcement/2018/suse-su-20180600-1.html");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1040151");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1077767");
  script_xref(name:"URL", value:"https://lists.suse.com/pipermail/sle-security-updates/2018-March/003783.html");
...
In default, this update would break a backwards compatibility with Puppet agents older than 3.2.2 as the SLE11 master doesn't support other fact formats than pson in default anymore.
In order to allow users to continue using their SLE11 agents a patch was added that enables sending PSON from agents.

For non-SUSE clients older that 3.2.2 a new puppet master boolean option
'dangerous_fact_formats' was added. When it's set to true it enables using dangerous fact formats (e.g. YAML). When it's set to false, only PSON fact format is accepted. (bsc#1040151),
(bsc#1077767)");

  script_tag(name:"affected", value:"'puppet' package(s) on SUSE Linux Enterprise Server 11-SP4, SUSE Linux Enterprise Server for SAP Applications 11-SP4.");
