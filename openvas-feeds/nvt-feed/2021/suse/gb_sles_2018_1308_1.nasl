...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.4.2018.1308.1");
  ...
  script_tag(name:"severity_date", value:"2018-03-22 16:50:31 +0000 (Thu, 22 Mar 2018)");

  script_name("SUSE: Security Advisory (SUSE-SU-2018:1308-1)");
  ...
  script_xref(name:"Advisory-ID", value:"SUSE-SU-2018:1308-1");
  script_xref(name:"URL", value:"https://www.suse.com/support/update/announcement/2018/suse-su-20181308-1.html");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1068032");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1076114");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1076179");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1082276");
  script_xref(name:"URL", value:"https://bugzilla.suse.com/1083291");
  script_xref(name:"URL", value:"https://lists.suse.com/pipermail/sle-security-updates/2018-May/004054.html");
  script_xref(name:"URL", value:"https://www.qemu.org/2018/02/14/qemu-2-11-1-and-spectre-update/");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'kvm' package(s) announced via the SUSE-SU-2018:1308-1 advisory.");
...
which now integrates with corresponding changes in libvirt. A January 2018 release of qemu initially addressed the Spectre v2 vulnerability for KVM guests by exposing the spec-ctrl feature for all x86 vcpu types, which was the quick and dirty approach,
but not the proper solution. We remove that initial patch and now rely on patches from upstream. This update defines spec_ctrl and ibpb cpu feature flags as well as new cpu models which are clones of existing models with either -IBRS or -IBPB added to the end of the model name. These new vcpu models explicitly include the new feature(s), whereas the feature flags can be added to the cpu parameter as with other features. In short, for continued Spectre v2 protection, ensure that either the appropriate cpu feature flag is added to the QEMU command-line, or one of the new cpu models is used. Although migration from older versions is supported, the new cpu features won't be properly exposed to the guest until it is restarted with the cpu features explicitly added. A reboot is insufficient.
...
if(release == "SLES11.0SP3") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"kvm", rpm:"kvm~1.4.2~53.17.1", rls:"SLES11.0SP3"))) {
    report += res;
  }
