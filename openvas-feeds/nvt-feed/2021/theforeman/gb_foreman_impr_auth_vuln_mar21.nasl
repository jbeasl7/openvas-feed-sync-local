...
if (description)
{
  ...
  script_tag(name:"summary", value:"Foreman is prone to an improper authorization
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The SmartProxyAuth of the Foreman allows controllers to
  authenticate certain requests based on the client certificate. As Puppet CA will consider subject
  alternative names (SANs) from a certificate along with Common name (CN), Puppet CA will sign the
  certificate with SANs pointing at DNS names of the already existing certificate.");
