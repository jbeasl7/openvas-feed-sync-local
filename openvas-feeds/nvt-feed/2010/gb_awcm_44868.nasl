...
if(description)
{
  ...
  script_name("AWCM CMS <= 2.2 Multiple RFI Vulnerabilities");
  ...
  script_tag(name:"summary", value:"AWCM CMS is prone to multiple remote file include (RFI)
  vulnerabilities because it fails to properly sanitize user-supplied input.");

  script_tag(name:"vuldetect", value:"Sends a crafted HTTP GET request and checks the response.");

  script_tag(name:"impact", value:"An attacker can exploit these vulnerabilities to obtain
  potentially sensitive information or to execute arbitrary script code in the context of the
  webserver process. This may allow the attacker to compromise the application and the computer,
  other attacks are also possible.");

  script_tag(name:"affected", value:"AWCM version 2.1, 2.2 and probably prior.");
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/44868");
...
foreach dir (make_list_unique("/awcm", "/cms", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/includes/window_top.php");
  if (res !~ "^HTTP/1\.[01] 200")
    continue;

  foreach file (keys(files)) {
    url = dir + "/includes/window_top.php?theme_file=" + crap(data: "../", length: 3 * 9) +
          files[file] + "%00";
