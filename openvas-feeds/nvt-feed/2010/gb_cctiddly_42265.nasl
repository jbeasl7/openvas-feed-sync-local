...
if(description)
{
  ...
  script_name("ccTiddly <= 1.7.6 Multiple RFI Vulnerabilities");
  ...
  script_tag(name:"summary", value:"ccTiddly is prone to multiple remote file include (RFI)
  ...
  script_tag(name:"affected", value:"ccTiddly version 1.7.6 and prior.");
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/42265");
...
foreach dir (make_list_unique("/cctiddly", http_cgi_dirs(port: port))) {
  if (dir == "/")
    dir = "";

  res = http_get_cache(port: port, item: dir + "/includes/include.php");
  if (res !~ "^HTTP/1\.[01] 200")
    continue;

  foreach file (keys(files)) {
    url = dir + "/includes/include.php?cct_base=../../../../../../../../../" + files[file] + "%00";
