...
if(description)
{
  ...
  script_name("LittlePhpGallery <= 1.0.2 LFI Vulnerability");
  ...
  script_tag(name:"summary", value:"LittlePhpGallery is prone to a local file include (LFI)
  ...
  script_tag(name:"affected", value:"LittlePhpGallery version 1.0.2 and probably prior.");
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/45143");
...
foreach dir (make_list_unique("/", "/gallery", "/images", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/gallery.php");
  if (res !~ "^HTTP/1\.[01] 200")
    continue;

  foreach file (keys(files)) {
    url = dir + "/gallery.php?repertoire=" + crap(data: "../", length: 3 * 9) + files[file] + "%00";
