...
if(description)
{
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/40409");
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/40559");
...
foreach i (list)
{
  exePath = registry_get_sz(key:"SOFTWARE\Microsoft\Office\" + i + "\Common\InstallRoot", item:"Path");
  if(exePath)
  {
    exeVer = fetch_file_version(sysPath:exePath, file_name:"INFOPATH.EXE");
    if(exeVer)
    {
      if(version_in_range(version:exeVer, test_version:"11.0", test_version2:"11.0.8232.0") ||
