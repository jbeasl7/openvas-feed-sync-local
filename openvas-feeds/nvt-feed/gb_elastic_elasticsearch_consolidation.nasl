...
if (description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.125293");
  ...
  script_name("Elastic Elasticsearch Detection Consolidation");

  script_tag(name:"summary", value:"Consolidation of Elastic Elasticsearch detections.");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2025 Greenbone AG");
  script_family("Product detection");
  script_dependencies("gb_elastic_elasticsearch_http_detect.nasl");
  if(FEED_NAME == "GSF" || FEED_NAME == "GEF" || FEED_NAME == "SCM")
    script_dependencies("gsf/gb_elastic_elasticsearch_smb_login_detect.nasl");
...
if( ! get_kb_item( "elastic/elasticsearch/detected" ) )
  exit( 0 );
...
foreach source( make_list("smb-login", "http" ) ) {
  ...
  install_list = get_kb_list( "elastic/elasticsearch/" + source + "/*/installs" );
  if( ! install_list )
    continue;
  ...
  foreach install( install_list ) {
    ...
    cpe1 = build_cpe( value:version, exp:"^([0-9.]+)", base:"cpe:/a:elastic:elasticsearch:" );
    cpe2 = build_cpe( value:version, exp:"^([0-9.]+)", base:"cpe:/a:elasticsearch:elasticsearch:" );
    if( ! cpe1 ) {
      cpe1 = "cpe:/a:elastic:elasticsearch";
      cpe2 = "cpe:/a:elasticsearch:elasticsearch";
    }
