...
if(description)
{
  ...
  script_name("Ruby on Rails (RoR) Detection Consolidation");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2020 Greenbone AG");
  script_family("Product detection");
  script_dependencies("gb_ruby_rails_ssh_login_detect.nasl", "gb_ruby_rails_http_detect.nasl");
  script_mandatory_keys("rails/detected");

  script_tag(name:"summary", value:"Consolidation of Ruby on Rails (RoR) detections.");
...
foreach proto( make_list( "ssh-login", "http" ) ) {
  ...
  foreach install( install_list ) {
    ...
    report += build_detection_report( app: "Ruby on Rails (RoR)",
                                      version: version,
                                      install: location,
                                      cpe: cpe,
                                      concluded: concluded,
