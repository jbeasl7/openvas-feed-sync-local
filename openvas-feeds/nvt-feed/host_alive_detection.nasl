...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://docs.greenbone.net/GSM-Manual/gos-21.04/en/upgrading-gos.html#boreas-alive-scanner");
  ...
  script_tag(name:"vuldetect", value:"Note: If the Boreas Host Alive Scanner is enabled (which is
  the default in recent and supported versions of the Greenbone software stack) this VT is only used
  for reporting if a host has been found as alive (if 'Report about reachable Hosts' has been
  enabled in the script preferences). No actual determination if a host is alive takes place in this
  case. Please see the reference for more information.

  Several methods can be configured for detecting whether a host is alive:
...
# In the following line, unreachable is spelled incorrectly.
...
# nb: In the following line, unreachable is spelled incorrectly.
# Unfortunately, this must stay for now in order to keep compatibility with existing scan configs.
