...
if (description)
{
  ...
  script_tag(name:"summary", value:"GitLab is prone to a vulnerability in the Jupyter
  Notebooks.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Missing sanitization of HTML attributes in Jupyter notebooks
  allows an attacker to perform arbitrary HTTP POST requests on a user's behalf leading to potential
  account takeover.");

  script_tag(name:"affected", value:"GitLab version 14.5.x through 14.5.3, 14.6.x through 14.6.3
  and 14.7.0.");
