...
if (description)
{
  ...
  script_tag(name:"insight", value:"When renaming a file, the file header shows a diff between the
  new and the old name on the /frans/test/merge_requests/3/diffs-page. When the diff is in the name
  (file was moved) the names of the file is not sanitized properly, triggering javascript inside the
  filename when accessing the /diffs-endpoint.");

  script_tag(name:"affected", value:"GitLab version 8.4.x through 10.4.6, 10.5.x through 10.5.6
  and 10.6.x through 10.6.2.");
