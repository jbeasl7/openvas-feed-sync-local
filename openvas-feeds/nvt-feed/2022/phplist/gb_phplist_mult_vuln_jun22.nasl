...
if (description)
{
  ...
  script_tag(name:"severity_date", value:"2022-06-17 14:13:00 +0000 (Fri, 17 Jun 2022)");
  ...
  injection. The injection takes place into an UPDATE, so the easiest way to extract data is via
  error based SQL injection.

  - CVE-2017-20031: When viewing users, the sortby parameter can be used to sort the list. The drop
  down list allows sorting by email, dates, and so on. All non-word characters are removed, but
  there are no further checks.

  - CVE-2017-20032: When subscribing a user, metadata is saved in the database. When saving this
  data in the database, it is neither properly escaped nor are prepared statements used, but the
  ...
  - CVE-2017-20035: Various parameters of the subscribe page - such as the title - are vulnerable to
  persistent XSS. An account with the privilege to edit the subscribe page is required.
