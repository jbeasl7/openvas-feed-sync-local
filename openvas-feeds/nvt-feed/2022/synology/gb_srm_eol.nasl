...
if(description)
{
  ...
  script_tag(name:"summary", value:"The Synology Router Manager (SRM) version on the remote host has
  reached the End of Life (EOL) and should not be used anymore.");

  script_tag(name:"vuldetect", value:"Checks if an EOL version is present on the target host.");

  script_tag(name:"impact", value:"An EOL version of SRM is not receiving any security updates from
  the vendor. Unfixed security vulnerabilities might be leveraged by an attacker to compromise the
  security of this host.");

  script_tag(name:"solution", value:"Update the SRM version on the remote host to a still supported
  version.");
...
if (ret = product_reached_eol(cpe: CPE, version: version)) {
  report = eol_build_message(name: "Synology Router Manager (SRM)",
                             cpe: CPE,
                             version: version,
