...
if(description)
{
  ...
  script_tag(name:"summary", value:"Apple iTunes is prone to a stack based buffer overflow
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Error occurs when application fails to perform adequate
  boundary checks before copying user-supplied data to an insufficiently-sized buffer while
  processing a specially crafted 'itms:' URL.");

  script_tag(name:"impact", value:"Successful attacks will lead to buffer overflow or denial of
  service to legitimate users.");

  script_tag(name:"affected", value:"Apple iTunes prior to version 8.2 on Windows.");
...
if(version_is_less(version:version, test_version:"8.2.0.23")) {
  report = report_fixed_ver(installed_version:version, fixed_version:"8.2.0.23", install_path:location);
