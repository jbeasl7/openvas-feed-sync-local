...
if (description)
{
  ...
  script_dependencies("gb_moxa_mgate_http_detect.nasl",
                      "gb_moxa_mgate_telnet_detect.nasl");
  if (FEED_NAME == "GSF" || FEED_NAME == "GEF" || FEED_NAME == "SCM")
    script_dependencies("gsf/gb_moxa_mgate_ethernetip_detect.nasl");
  script_mandatory_keys("moxa/mgate/detected");

  script_tag(name:"summary", value:"Consolidation of Moxa MGate detections.");
...
foreach source (make_list("http", "telnet", "ethernetip")) {
...
if (http_ports = get_kb_list("moxa/mgate/http/port")) {
  foreach port (http_ports) {
    ...
    conclUrl = get_kb_item("moxa/mgate/http/" + port + "/concludedUrl");
    if (conclUrl)
...
if (ethernetip_ports = get_kb_list("moxa/mgate/ethernetip/port")) {
  foreach port (ethernetip_ports) {
    if (ethernetip_protos = get_kb_list("moxa/mgate/ethernetip/" + port + "/proto")) {
      foreach proto (ethernetip_protos) {
        extra += "EtherNet/IP on port " + port + "/" + proto + '\n';

        concluded = get_kb_item("moxa/mgate/ethernetip/" + port + "/" + proto + "/concluded");
        if (concluded)
          extra += concluded + '\n';
