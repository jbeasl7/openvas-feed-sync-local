...
if(description)
{
  ...
  script_name("vBulletin <= 5.3.4 Arbitrary File Deletion And RCE Vulnerabilities");

  script_tag(name:"summary", value:"vBulletin is prone to arbitrary file deletion and remote code
  ...
  - Unsafe usage of PHP's unserialize function in vB_Library_Template's cacheTemplates function,
  which is a publicly exposed API.

  - A deserialization vulnerability.");

  script_tag(name:"impact", value:"Successfully exploiting this issue allow remote attackers to
  execute arbitrary code execution and arbitrary file deletion on the affected system.");
