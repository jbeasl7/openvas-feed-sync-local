...
if (description)
{
  ...
  script_name("OTRS 3.3.x <= 3.3.16, 4.x <= 4.0.23, 5.x <= 5.0.19 Privilege Escalation Vulnerability");
  ...
  script_tag(name:"insight", value:"An attacker with agent permission is capable by opening a
  specific URL in a browser to gain administrative privileges / full access. Afterward, all system
  settings can be read and changed.");

  script_tag(name:"affected", value:"OTRS version 3.3.x through 3.3.16, 4.x through 4.0.23 and 5.x
  through 5.0.19.");

  script_tag(name:"solution", value:"Update to version 3.3.17, 4.0.24, 5.0.20 or later.");
...
if (version_in_range(version: version, test_version: "4.0.0", test_version2: "4.0.23")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "4.0.24", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}
