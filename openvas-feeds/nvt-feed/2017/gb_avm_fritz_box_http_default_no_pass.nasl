...
if(description)
{
  ...
  script_dependencies("gb_avm_fritz_box_consolidation.nasl", "gb_default_credentials_options.nasl");
...
function do_webcm_post_req( port, posturl, sid, dir ) {
  ...
  req = http_post_put_req( port:port, url:posturl, data:postdata,
                       ...
                       add_headers:make_array( "Content-Type", "application/x-www-form-urlencoded",
                                               ...
                                               "Referer", http_report_vuln_url( port:port, url:dir + "/cgi-bin/webcm?getpage=../html/index_inhalt.html", url_only:TRUE ) ) );
    ...
    if( ! isnull( sid[1] ) ) {
      ...
      if( ! isnull( challenge[1] ) ) {
        ...
        req = http_post_put_req( port:port, url:posturl, data:postdata,
                             ...
                             add_headers:make_array( "Content-Type", "application/x-www-form-urlencoded",
                                                     "Upgrade-Insecure-Requests", "1",
                                                     "Referer", http_report_vuln_url( port:port, url:dir + "/cgi-bin/webcm", url_only:TRUE ) ) );
        res = http_send_recv( port:port, data:req );
