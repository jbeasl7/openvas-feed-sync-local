...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2026.1229");
  ...
  script_name("Huawei EulerOS: Security Advisory for avahi (EulerOS-SA-2026-1229)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2026-1229");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/intl/en-us/euleros/securitydetail.html?secId=EulerOS-SA-2026-1229");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'avahi' package(s) announced via the EulerOS-SA-2026-1229 advisory.");
...
Avahi is a system which facilitates service discovery on a local network via the mDNS/DNS-SD protocol suite. In 0.9-rc2 and earlier, avahi-daemon can be crashed by sending unsolicited announcements containing CNAME resource records pointing it to resource records with short TTLs. As soon as they expire avahi-daemon crashes.(CVE-2025-68468)");
  ...
  script_tag(name:"affected", value:"'avahi' package(s) on Huawei EulerOS V2.0SP13(x86_64).");
...
if(release == "EULEROS-2.0SP13-x86_64") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"avahi-libs", rpm:"avahi-libs~0.8~18.h4.eulerosv2r13", rls:"EULEROS-2.0SP13-x86_64"))) {
    report += res;
  }
