...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2026.1237");
  ...
  script_name("Huawei EulerOS: Security Advisory for glib-networking (EulerOS-SA-2026-1237)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2026-1237");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/intl/en-us/euleros/securitydetail.html?secId=EulerOS-SA-2026-1237");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'glib-networking' package(s) announced via the EulerOS-SA-2026-1237 advisory.");
...
glib-networking's OpenSSL backend fails to properly check the return value of memory allocation routines. An out of memory condition could potentially result in writing to an invalid memory location.(CVE-2025-60019)");
  ...
  script_tag(name:"affected", value:"'glib-networking' package(s) on Huawei EulerOS V2.0SP13(x86_64).");
...
if(release == "EULEROS-2.0SP13-x86_64") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"glib-networking", rpm:"glib-networking~2.68.1~5.h1.eulerosv2r13", rls:"EULEROS-2.0SP13-x86_64"))) {
    report += res;
  }
