...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.2.2026.1585");
  ...
  script_name("Huawei EulerOS: Security Advisory for libwebsockets (EulerOS-SA-2026-1585)");
  ...
  script_xref(name:"Advisory-ID", value:"EulerOS-SA-2026-1585");
  script_xref(name:"URL", value:"https://developer.huaweicloud.com/intl/en-us/euleros/securitydetail.html?secId=EulerOS-SA-2026-1585");

  script_tag(name:"summary", value:"The remote host is missing an update for the Huawei EulerOS 'libwebsockets' package(s) announced via the EulerOS-SA-2026-1585 advisory.");
...
Stack-based Buffer Overflow in lws_adns_parse_label in warmcat libwebsockets allows, when the LWS_WITH_SYS_ASYNC_DNS flag is enabled during compilation, to overflow the label_stack, when the attacker is able to sniff a DNS request in order to craft a response with a matching id containing a label longer than the maximum.(CVE-2025-11678)");
  ...
  script_tag(name:"affected", value:"'libwebsockets' package(s) on Huawei EulerOS V2.0SP11(x86_64).");
...
if(release == "EULEROS-2.0SP11-x86_64") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"libwebsockets", rpm:"libwebsockets~4.3.0~2.h9.eulerosv2r11", rls:"EULEROS-2.0SP11-x86_64"))) {
    report += res;
  }
