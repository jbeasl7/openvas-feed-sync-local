...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.4.2025.02564.1");
  ...
  script_name("SUSE: Security Advisory (SUSE-SU-2025:02564-1)");
...
- CVE-2025-32441: Fixed a bug where simultaneous rack requests can restore a deleted rack session. (bsc#1242898)");
  ...
  script_tag(name:"affected", value:"'rmt-server' package(s) on SUSE Linux Enterprise Server 15-SP5, SUSE Linux Enterprise Server for SAP Applications 15-SP5.");
...
if(release == "SLES15.0SP5") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"rmt-server", rpm:"rmt-server~2.23~150500.3.34.2", rls:"SLES15.0SP5"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"rmt-server-config", rpm:"rmt-server-config~2.23~150500.3.34.2", rls:"SLES15.0SP5"))) {
    report += res;
  }
