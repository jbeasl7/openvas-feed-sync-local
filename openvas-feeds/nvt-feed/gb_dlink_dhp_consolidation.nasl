...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.170779");
  script_version("2025-03-04T05:38:25+0000");
  script_tag(name:"last_modification", value:"2025-03-04 05:38:25 +0000 (Tue, 04 Mar 2025)");
  script_tag(name:"creation_date", value:"2024-05-07 08:34:41 +0000 (Tue, 07 May 2024)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:N/AC:L/Au:N/C:N/I:N/A:N");

  script_tag(name:"qod_type", value:"remote_banner");

  script_name("D-Link DHP Device Detection Consolidation");
  ...
  script_mandatory_keys("d-link/dhp/detected");

  script_tag(name:"summary", value:"Consolidation of D-Link DHP devices detections.");
...
if( ! get_kb_item( "d-link/dhp/detected" ) )
  exit( 0 );
...
foreach source( make_list( "http", "hnap", "upnp", "mdns" ) ) {
  version_list = get_kb_list( "d-link/dhp/" + source + "/*/fw_version" );
  ...
  hw_version_list = get_kb_list( "d-link/dhp/" + source + "/*/hw_version" );
  ...
  model_list = get_kb_list( "d-link/dhp/" + source + "/*/model" );
...
os_app = "D-Link DHP";
os_cpe = "cpe:/o:dlink:dhp";
hw_app = "D-Link DHP";
hw_cpe = "cpe:/h:dlink:dhp";
...
if( detected_model != "unknown" ) {
  ...
  set_kb_item( name:"d-link/dhp/model", value:"DHP-" + detected_model );
} else {
...
if( fw_version != "unknown" ) {
  os_cpe += ":" + fw_version;
  set_kb_item( name:"d-link/dhp/fw_version", value:fw_version );
}

if( hw_version != "unknown" ) {
  hw_cpe += ":" + tolower( hw_version );
  set_kb_item( name:"d-link/dhp/hw_version", value:hw_version );
}

register_port = 0;

if( http_ports = get_kb_list( "d-link/dhp/http/port" ) ) {
  foreach port( http_ports ) {
    detection_methods += '\nHTTP(s) on port ' + port + '/tcp\n';
    fw_concluded = get_kb_item( "d-link/dhp/http/" + port + "/fw_concluded" );
    fw_conclurl = get_kb_item( "d-link/dhp/http/" + port + "/fw_conclurl" );
    if( fw_concluded && fw_conclurl )
      detection_methods += '  Firmware concluded:\n    ' + fw_concluded + '\n  from URL(s):\n    ' + fw_conclurl + '\n';
    else if( fw_concluded )
      detection_methods += '  Firmware concluded:\n    ' + fw_concluded + '\n';

    hw_concluded = get_kb_item( "d-link/dhp/http/" + port + "/hw_concluded" );
    hw_conclurl = get_kb_item( "d-link/dhp/http/" + port + "/hw_conclurl" );
...
if( http_ports = get_kb_list( "d-link/dhp/hnap/port" ) ) {
  foreach port( http_ports ) {

    detection_methods += '\nHNAP on port ' + port + '/tcp\n';

    fw_concluded = get_kb_item( "d-link/dhp/hnap/" + port + "/fw_concluded" );

    if( fw_concluded )
      detection_methods += "  Firmware concluded:  " + fw_concluded + '\n';

    hw_concluded    = get_kb_item( "d-link/dhp/hnap/" + port + "/hw_concluded" );
    if( hw_concluded )
      detection_methods += "  Hardware version concluded:  " + hw_concluded + '\n';

    conclurl = get_kb_item( "d-link/dhp/hnap/" + port + "/conclurl" );
    if( conclurl )
...
if( mdns_ports = get_kb_list( "d-link/dhp/mdns/port" ) ) {
  foreach port( mdns_ports ) {
    mdns_port_and_proto = get_kb_item( "d-link/dhp/mdns/" + port + "/mdns_port_and_proto" );

    detection_methods += '\nmDNS on port ' + mdns_port_and_proto + " exposing service for " + port + '/tcp\n';

    concluded = get_kb_item( "d-link/dhp/mdns/" + port + "/concluded" );
    if( concluded )
...
if( upnp_ports = get_kb_list( "d-link/dhp/upnp/port" ) ) {
  foreach port( upnp_ports ) {
    detection_methods += '\nUPnP on port ' + port + '/tcp\n';

    concluded = get_kb_item( "d-link/dhp/upnp/" + port + "/concluded" );
    if ( concluded ) {
      detection_methods += '  Concluded:' + concluded;
      concludedurl = get_kb_item( "d-link/dhp/upnp/" + port + "/concludedUrl" );
      if ( concludedurl )
        detection_methods += '\n  from URL:\n    ' + concludedurl;
    }
  }
