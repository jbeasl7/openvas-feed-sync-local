...
if(description)
{
  ...
  script_name("Niagara Fox (Flexible Object eXchange) Service Detection");
  ...
  script_tag(name:"summary", value:"A Niagara Fox (Flexible Object eXchange) service is running at
  this host.");

  script_tag(name:"insight", value:"The Fox protocol, developed as part of the Niagara framework
  from Tridium, is most commonly used in building automation systems.");
...
foreach line (lines) {
  ...
  if (!isnull(buf[1])) {
    app_name = buf[1];
    set_kb_item(name: "niagara_fox/" + port + "/app_name", value: app_name);
    continue;
  }
  ...
  if (!isnull(buf[1])) {
    app_version = buf[1];
    set_kb_item(name: "niagara_fox/" + port + "/app_version", value: app_version);
    continue;
  }
  ...
  if (!isnull(buf[1])) {
    os_name = buf[1];
    set_kb_item(name: "niagara_fox/" + port + "/os_name", value: os_name);
    continue;
  }
  ...
  if (!isnull(buf[1])) {
    os_version = buf[1];
    set_kb_item(name: "niagara_fox/" + port + "/os_version", value: os_version);
    continue;
  }
  ...
  if (!isnull(buf[1])) {
    brandId = buf[1];
    set_kb_item(name: "niagara_fox/" + port + "/brandId", value: brandId);
    continue;
  }
}

service_register(port: port, proto: "niagara-fox");
