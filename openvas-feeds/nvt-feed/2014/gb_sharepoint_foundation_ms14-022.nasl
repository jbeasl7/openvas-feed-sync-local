...
if(description)
{
  ...
  script_mandatory_keys("microsoft/sharepoint/foundation/smb-login/detected");
...
if(shareVer =~ "^14\.")
{
  ...
  if(dllVer)
  {
    ...
    {
      report = report_fixed_ver(installed_version:dllVer, vulnerable_range:"14.0 - 14.0.7123.4999", install_path:path);
...
if(shareVer =~ "^15\.")
{
  ...
  if(path)
  {
    ...
    if(dllVer2)
    {
      ...
      {
        report = report_fixed_ver(installed_version:dllVer2, vulnerable_range:"15.0 - 15.0.4615.999", install_path:path);
        security_message(port:port, data:report);
        exit(0);
      }
    }
