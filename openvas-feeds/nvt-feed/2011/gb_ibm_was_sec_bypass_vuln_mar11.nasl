...
if(description)
{
  ...
  script_name("IBM WebSphere Application Server Security Bypass Vulnerability (Mar 2011)");
  ...
  which does not prevent modifications of the primary admin id, allows remote authenticated
  administrators to bypass intended access restrictions by mapping a 'user' or 'group' to an
  administrator role.");

  script_tag(name:"impact", value:"Successful exploitation will let remote authenticated
  administrators to bypass intended access restrictions.");

  script_tag(name:"affected", value:"IBM WebSphere Application Server version 6.1.x prior to
  6.1.0.31 and 7.x prior to 7.0.0.15.");

  script_tag(name:"solution", value:"Update to version 6.1.0.31, 7.0.0.15 or later.");
