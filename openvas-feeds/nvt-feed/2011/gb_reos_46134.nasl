...
if (description)
{
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/46134");
...
foreach dir( make_list_unique( "/reos", http_cgi_dirs( port:port ) ) ) {

  if( dir == "/" ) dir = "";
  buf = http_get_cache(item:dir + "/index.php", port:port);
  if(!buf || buf !~ "^HTTP/1\.[01] 200" || ('content="real state, for sale' >!< buf && "Powered by" >!< buf && ">eLazos</a>" >!< buf))
    continue;
