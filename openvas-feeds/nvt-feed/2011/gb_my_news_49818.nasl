...
if (description)
{
  ...
  script_name("MyNews <= 1.2 Multiple RFI Vulnerabilities - Active Check");
  ...
  script_tag(name:"summary", value:"MyNews 1.2 is prone to multiple remote file inclusion (RFI)
  ...
  script_tag(name:"affected", value:"MyNews version 1.2 and prior.");
...
foreach dir( make_list_unique( "/mynews", "/news", http_cgi_dirs( port:port ) ) ) {
  ...
  res = http_get_cache( port:port, item:dir + "/includes/tiny_mce/plugins/filemanager/classes/FileManager/FileSystems/ZipFileImpl.php" );
