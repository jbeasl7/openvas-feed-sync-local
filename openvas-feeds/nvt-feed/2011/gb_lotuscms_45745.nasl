...
if(description)
{
  ...
  script_name("LotusCMS <= 3.0 LFI Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"LotusCMS is prone to a local file include (LFI) vulnerability
  ...
  script_tag(name:"affected", value:"LotusCMS version 3.0 and probably prior.");
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/45745");
...
foreach dir( make_list_unique( "/lcms", "/cms", http_cgi_dirs( port:port ) ) ) {
  ...
  foreach file( keys( files ) ) {
    ...
    url = dir + "/index.php?system=" + crap( data:"../", length:3 * 9 ) + files[file] + "%00";
