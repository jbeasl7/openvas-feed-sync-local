...
if (description)
{
  ...
  script_name("Betsy <= 4.0 LFI Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"Betsy is prone to a local file include (LFI) vulnerability
  ...
  script_tag(name:"affected", value:"Betsy version 4.0 and prior.");
  ...
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/46124");
...
foreach dir( make_list_unique( "/cms", "/betsy", http_cgi_dirs( port:port ) ) ) {

  if( dir == "/" )
    dir = "";

  res = http_get_cache( port:port, item:dir + "/ress.php" );
