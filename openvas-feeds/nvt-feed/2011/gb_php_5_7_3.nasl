...
if(description)
{
  ...
  script_name("PHP 5.3.7 Cryptographic Vulnerability");
  ...
  script_tag(name:"summary", value:"PHP is prone to a cryptographic vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The crypt function in PHP 5.3.7, when the MD5 hash type is
  used, returns the value of the salt argument instead of the hashed string.");

  script_tag(name:"impact", value:"The vulnerability might allow remote attackers to bypass
  authentication via an arbitrary password.");

  script_tag(name:"affected", value:"PHP version 5.3.7 only.");
