...
if (description)
{
  ...
  script_name("Portix-CMS <= 1.5.0.rc5 LFI Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"Portix-CMS is prone to a local file include (LFI) vulnerability
  ...
  script_tag(name:"affected", value:"Portix-CMS version 1.5.0.rc5 and prior.");
...
foreach dir( make_list_unique("/", "/portix", "/cms", http_cgi_dirs( port:port ) ) ) {
  if( dir == "/" )
    dir = "";

  res = http_get_cache( port:port, item:dir + "/print.php" );
