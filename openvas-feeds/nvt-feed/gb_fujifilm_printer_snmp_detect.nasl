...
if (description)
{
  ...
  script_tag(name:"summary", value:"SNMP based detection of Fuji Xerox / Fujifilm printer
  devices.");
...
if(sysdesc =~ "^(FUJI XEROX|FUJIFILM) ") {
  ...
  if(!isnull(mod[2])) {
    if(";" >!< mod[0] && sysdesc !~ "FUJIFILM") {
      ...
      # nb: As seen in the example, we need to detect this kind of cases and fix them
      # FUJI XEROX DocuPrint CP225/228 w; ESS 01.10.00.20.3938, IOT <foo> 01.03.00
      # FUJI XEROX DocuPrint CM225/228 fw; ESS 01.02.00.20.3836, IOT <foo> 01.02.00
      model = mod[2];
      if (model =~ "/[0-9]+ ")
        model = ereg_replace(string: model, pattern: "(/[0-9]+)", replace: "");
      set_kb_item(name: "fujifilm/printer/snmp/" + port + "/model", value: model);
    }
  }
  ...
  # nb: This was determined using snmpwalk and version format similarity with other devices affected by same class of vulnerabilities
  # 01.00.41.20.0832
  ver_oid = "1.3.6.1.4.1.297.1.111.1.31.11";
  v = snmp_get(port: port, oid: ver_oid);

  if(!isnull(v)) {
    set_kb_item(name: "fujifilm/printer/snmp/" + port + "/fw_version", value: v);
    set_kb_item(name: "fujifilm/printer/snmp/" + port + "/concludedFwOID", value: ver_oid);
    exit(0);
  }
