...
if(description)
{
  ...
  script_name("Zavio IP Cameras Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of Zavio IP Cameras.");
...
if(res =~ 'initProdNbr="([^"]+)";' && (res =~ 'BrandCopyright="Zavio\\s*";' || res =~ 'initBrand="Zavio\\s*";')) {
  ...
  os_register_and_report( os:os_name, cpe:os_cpe, desc:"Zavio IP Cameras Detection (HTTP)", runs_key:"unixoide" );
