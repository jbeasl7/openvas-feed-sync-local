...
if( un = egrep( string:uname, pattern:"Welcome to pfSense", icase:TRUE ) ) {
  ...
  set_kb_item( name:"pfsense/ssh-login/port", value:port);
  set_kb_item( name:"ssh/force/pty", value:TRUE );
...
if( rls =~ "EulerOS release" ) {
  ...
  # Remove inactive kernel packages from package list (GS-8817)
  active_kernel_package = "kernel-" + ssh_cmd( socket:sock, cmd:"uname -r" );
  new_buf = "";
  foreach line ( split( buf, keep:TRUE ) ) {
    # Non-kernel package
    if (!eregmatch( string:line, pattern:"^kernel-[0-9]")) {
      new_buf += line;
      continue;
    }

    # Kernel package, but from inactive kernel
    if (chomp( line ) != active_kernel_package) {
      continue;
    }

    # Package matches currently active kernel
    new_buf += line;
  }
  new_buf = chomp( new_buf );

  if( new_buf ) {
    if( ! register_notus( os_release:notus_os_release, pkg_list:new_buf, rpms:TRUE ) )
      error = new_buf;
  }
...
if( "Darwin" >< uname ) {
  ...
  if( sw_vers_buf )
    ...
    detect_text = "Apple macOS / Mac OS X";
  log_message( port:0, data:create_lsc_os_detection_report( detect_text:detect_text ) );
