...
if(description)
{
  ...
  script_tag(name:"summary", value:"SMB login-based detection of Apple Safari.");
...
foreach item (registry_enum_keys(key:key))
{
  ...
  if("Safari" >< safariName)
  {
    ...
    if(safariPath)
    {
      ...
      if(safariVer)
      {
        set_kb_item(name:"apple/safari/detected", value:TRUE);
        set_kb_item(name:"apple/safari/smb-login/detected", value:TRUE);

        cpe = build_cpe(value:safariVer, exp:"^([0-9.]+)", base:"cpe:/a:apple:safari:");
        if(!cpe)
          cpe = "cpe:/a:apple:safari";

        register_product(cpe:cpe, location:safariPath, port:0, service:"smb-login");
