# SPDX-FileCopyrightText: 2019 Greenbone AG
# Some text descriptions might be excerpted from (a) referenced
# source(s), and are Copyright (C) by the respective right holder(s).
#
# SPDX-License-Identifier: GPL-2.0-only

if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.108531");
  script_version("2025-09-05T15:40:40+0000");
  script_tag(name:"last_modification", value:"2025-09-05 15:40:40 +0000 (Fri, 05 Sep 2025)");
  script_tag(name:"creation_date", value:"2019-01-20 14:05:39 +0100 (Sun, 20 Jan 2019)");
  script_tag(name:"cvss_base", value:"0.0");
  script_tag(name:"cvss_base_vector", value:"AV:N/AC:L/Au:N/C:N/I:N/A:N");

  script_tag(name:"qod_type", value:"remote_banner");

  script_name("Adminer Detection (HTTP)");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2019 Greenbone AG");
  script_family("Product detection");
  script_dependencies("find_service.nasl", "no404.nasl", "webmirror.nasl",
                      "DDI_Directory_Scanner.nasl", "gb_php_http_detect.nasl",
                      "global_settings.nasl");
  script_require_ports("Services/www", 8080); # nb: TurnKeyLAMP might also run on 12322
  script_exclude_keys("Settings/disable_cgi_scanning");

  script_tag(name:"summary", value:"HTTP based detection of Adminer.");

  script_xref(name:"URL", value:"https://www.adminer.org/");
  script_xref(name:"URL", value:"https://github.com/vrana/adminer");

  exit(0);
}

include("cpe.inc");
include("host_details.inc");
include("http_func.inc");
include("http_keepalive.inc");
include("list_array_func.inc");
include("port_service_func.inc");

port = http_get_port( default:8080 );

if( ! http_can_host_php( port:port ) )
  exit( 0 );

rootInstalled = FALSE;

foreach dir( make_list_unique( "/", "/adminer", http_cgi_dirs( port:port ) ) ) {

  if( rootInstalled )
    break;

  install = dir;
  if( dir == "/" )
    dir = "";

  url = dir + "/adminer.php";
  res = http_get_cache( port:port, item:url );

  if( res !~ "^HTTP/1\.[01] 200" || "<title>Login - Adminer</title>" >!< res ||
      ( "://www.adminer.org/'" >!< res || ("id='h1'>Adminer</a>" >!< res && "id='logo'>Adminer<" >!< res ) ) ) {
    url = dir + "/";
    res = http_get_cache( port:port, item:url );

    if( res !~ "^HTTP/1\.[01] 200" || "<title>Login - Adminer</title>" >!< res ||
        ( "://www.adminer.org/'" >!< res || ( "id='h1'>Adminer</a>" >!< res && "id='logo'>Adminer<" >!< res ) ) )
      continue;
  }

  version = "unknown";
  conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );

  set_kb_item( name:"adminer/detected", value:TRUE );
  set_kb_item( name:"adminer/http/detected", value:TRUE );

  if( install == "/" )
    rootInstalled = TRUE;

  # onload: partial(verifyVersion, '4.7.0', '?', '674453:67503')});
  # id='h1'>Adminer</a> <span class="version">4.7.0</span>
  #
  # onclick="bodyClick(event);" onload="verifyVersion('4.2.5');">
  # id='h1'>Adminer</a> <span class="version">4.2.5</span>

  vers = eregmatch( pattern:"verifyVersion(, '|\(')([^']+)'", string:res, icase:FALSE );
  if( ! isnull( vers[2] ) ) {
    version = vers[2];
    conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
  } else {
    vers = eregmatch( pattern:"verifyVersion(, '|\(')([^']+)'", string:res, icase:FALSE );
    if( ! isnull( vers[2] ) ) {
      version = vers[2];
      conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
    } else {
      vers = eregmatch( pattern:'class="version">([^<]+)<', string:res, icase:FALSE );
      if( ! isnull( vers[1] ) ) {
        version = vers[1];
        conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
      } else {
        vers = eregmatch( pattern:'class="version">([^<]+)<', string:res, icase:FALSE );
        if( ! isnull( vers[1] ) ) {
          version = vers[1];
          conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
        } else {
          # ?file=logo.png&amp;version=5.3.0
          vers = eregmatch( pattern:"\.(png|css)&amp;version=([0-9.]+)", string:res );
          if( ! isnull( vers[2] ) ) {
            version = vers[2];
            conclUrl = http_report_vuln_url( port:port, url:url, url_only:TRUE );
          }
        }
      }
    }
  }

  cpe = build_cpe( value:version, exp: "^([0-9.]+)", base:"cpe:/a:adminer:adminer:" );
  if( ! cpe )
    cpe = "cpe:/a:adminer:adminer";

  register_product( cpe:cpe, location:install, port:port, service:"www" );

  log_message( data:build_detection_report( app:"Adminer", version:version, install:install, cpe:cpe,
                                            concluded:vers[0], concludedUrl:conclUrl ),
               port:port );
}

exit( 0 );
