...
if(description)
{
  ...
  script_name("RIPS Scanner Detection (HTTP)");
  ...
  script_xref(name:"URL", value:"https://rips-scanner.sourceforge.net/");

  script_tag(name:"summary", value:"HTTP based detection of the RIPS scanner.");
...
foreach dir( make_list_unique( "/", "/rips", http_cgi_dirs( port:port ) ) ) {
  ...
  if( res && "RIPS - A static source code analyser for vulnerabilities in PHP scripts" >< res ) {
    ...
    set_kb_item( name:"rips/detected", value:TRUE );
    set_kb_item( name:"rips/http/detected", value:TRUE );
