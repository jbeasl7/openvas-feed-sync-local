...
if(description)
{
  ...
  script_name("Get Windows Firewall Profile Status over WMI - Windows");
  ...
  In this Test is currently only an Registry Test for the Microsoft Firewall realized.

  Later we will test over WMI the Namespace SecurityCenter\FirewallProduct and
  SecurityCenter2\FirewallProduct for third party Firewall Products. The WMI test can only used for
  Microsoft Client and not for Server Systems.");
