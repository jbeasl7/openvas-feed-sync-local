...
if(description)
{
  ...
  script_dependencies("GSHB/GSHB_WMI_OSInfo.nasl", "GSHB/GSHB_WMI_Antivir.nasl",
                      "GSHB/GSHB_SMB_UAC_Config.nasl", "GSHB/GSHB_WMI_EFS.nasl");
...
if( max_index(SecurityCenter2) <= 1 ){
  ...
  desc += 'des IT-Systems vor einer Infektion mit Schadsoftware getroffen wurden.\n';
  result = 'nicht erfüllt';
}else{
...
result = 'erfüllt';
AuditPolicy = powershell_wmi_query( namespace:"root\rsop\computer", properties:"Category",  query:"Select * from RSOP_AuditPolicy where precedence=1" );
if( AuditPolicy ){
  desc = 'Folgende Überwachungsrichtlinien sind auf dem Host aktiviert:\n';
  foreach line (split(AuditPolicy)){
    desc += line;
  }
  desc += '\n';
}else{
...
upr_cmd = "Get-Ciminstance -Namespace 'root\RSOP\Computer' -query 'Select * from RSOP_UserPrivilegeRight where precedence=1'  | Select-Object UserRight,AccountList  | foreach-object {$_.UserRight + '|' + ($_.AccountList | foreach-object { $([char]0x22) + $_ + $([char]0x22) })}";
UserPrivilegeRight = powershell_cmd( cmd:upr_cmd);
if( UserPrivilegeRight ){
  desc += 'Folgende Benutzerrechte sind zugewiesen:\n';
  desc += 'UserRight|AccountList\n';
  foreach line (split(UserPrivilegeRight)){
    desc += line;
  }
...
ps_cmd = "Get-Ciminstance -Namespace 'root\RSOP\Computer' -query 'Select AccountList from RSOP_UserPrivilegeRight where UserRight=''SeDenyInteractiveLogonRight'' AND precedence=1' | foreach-object {($_.AccountList | foreach-object{ $([char]0x22) + $_ + $([char]0x22) })}";
SeDenyInteractiveLogonRight = powershell_cmd( cmd:ps_cmd );
if( ! SeDenyInteractiveLogonRight ){
  ...
  foreach line (split(SeDenyInteractiveLogonRight)){
    desc += line;
  }
}

ps_cmd = "Get-Ciminstance -Namespace 'root\RSOP\Computer' -query 'Select AccountList from RSOP_UserPrivilegeRight where UserRight=''SeInteractiveLogonRight'' AND precedence=1' | foreach-object {($_.AccountList | foreach-object{ $([char]0x22) + $_ + $([char]0x22) })}";
SeInteractiveLogonRight = powershell_cmd( cmd:ps_cmd );
if( ! SeInteractiveLogonRight ){
  ...
  foreach line (split(SeInteractiveLogonRight)){
    desc += line;
  }
}
...
if( FEED_NAME == "GSF" || FEED_NAME == "GEF" || FEED_NAME == "SCM" ) {
  ...
  foreach service ( split( service_list ) ) {
    value = eregmatch( string:service, pattern:"(.*);(DPS|WDiSvcHost|WerSvc);(.*);(.*);(.*);(.*);" );
    name = value[2];
    start_mode = value[5];
    start_mode_desc += name + " : " + start_mode;
  }
}else{
  ps_cmd = "Get-CimInstance -query 'SELECT StartMode FROM Win32_Service WHERE Name=''DPS'' OR Name=''WDiSvcHost'' OR Name=''WerSvc''' | ForEach-Object { $_.name + ' : ' + $_.startmode }";
  services = powershell_cmd( cmd:ps_cmd );
  if( services ) {
    foreach service( split(services) ) {
      start_mode_desc += service;
    }
  }
}

if( start_mode_desc ){
  desc = start_mode_desc + '\n';
