...
if(description)
{
  ...
  script_dependencies("GSHB/GSHB_WMI_OSInfo.nasl", "GSHB/GSHB_M4_097.nasl", "gb_ms_ie_detect.nasl",
                      "lsc_options.nasl");
  ...
  und Prozessen, die durch Serversysteme auf Basis von Windows Server 2012 (R2) im Regelbetrieb
  verarbeitet bzw. gesteuert werden.");
...
GROUPUSER = powershell_wmi_user_groupuser();
...
if( ! disabled_win_cmd_exec ){
  Firewall_Private_Stat = win_run_cmd(cmd:Firewall_Private);
  ...
  Firewall_Public_Stat = win_run_cmd(cmd:Firewall_Public);
  ...
  Firewall_Domain_Stat = win_run_cmd(cmd:Firewall_Domain);
