...
if(description)
{
  ...
  script_dependencies("GSHB/GSHB_WMI_OSInfo.nasl", "GSHB/GSHB_WMI_Antivir.nasl", "GSHB/GSHB_SMB_UAC_Config.nasl", "GSHB/GSHB_WMI_EFS.nasl", "lsc_options.nasl");
...
if( ! disabled_win_cmd_exec ){
  res = win_run_cmd(cmd:"bcdedit /enum");
  if( res ){
...
if( ! disabled_win_cmd_exec ){
  RDP_User = win_run_cmd(cmd:'net localgroup "Remote Desktop Users"');
  Administrators = win_run_cmd(cmd:'net localgroup "Administrators"');
}
...
if( ExecutionPolicy == "AllSigned"){
  ...
  desc += 'Dies sollte gesetzt werden, um sicherzustellen, dass nur signierte skripte ausgeführt werden können.';
  result = 'nicht erfüllt';
}
...
if( SecureBoot == "1" ){
  ...
  if( ! disabled_win_cmd_exec ){
    BootOption = win_run_cmd(cmd:'type C:\\Windows\\Panther\\setupact.log|find /i "Detected boot environment"');
    if( BootOption ){
