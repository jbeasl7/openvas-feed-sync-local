...
if(description)
{
  ...
  script_name("ArubaOS / InstantOS Detection (SNMP)");
  ...
  script_tag(name:"summary", value:"SNMP based detection of ArubaOS / InstantOS.");
...
# nb: To distinguish between ArubaOS and InstantOS
#     OID for version information on ArubaOS but not InstantOS
oid = "1.3.6.1.4.1.14823.2.2.1.2.1.28.0";
if (snmp_get(port: port, oid: oid))
  os_name = "ArubaOS";
else
  os_name = "Aruba InstantOS";
...
os_register_and_report(os: os_name, cpe: os_cpe, banner_type: "SNMP sysDescr OID" , port: port, proto: "udp",
                       banner: sysdesc, runs_key: "unixoide", desc: "ArubaOS / InstantOS Detection (SNMP)");
