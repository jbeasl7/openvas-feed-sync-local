...
if(description)
{
  ...
  script_tag(name:"creation_date", value:"2015-09-08 11:23:16 +0000 (Tue, 08 Sep 2015)");
  ...
  script_xref(name:"Advisory-ID", value:"ALAS-2014-353");
  script_xref(name:"URL", value:"https://alas.aws.amazon.com/ALAS-2014-353.html");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'libmicrohttpd' package(s) announced via the ALAS-2014-353 advisory.");
...
The MHD_http_unescape function in libmicrohttpd before 0.9.32 might allow remote attackers to obtain sensitive information or cause a denial of service (crash) via unspecified vectors that trigger an out-of-bounds read.");
  ...
  script_tag(name:"affected", value:"'libmicrohttpd' package(s) on Amazon Linux.");
...
if(release == "AMAZON") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"libmicrohttpd-debuginfo", rpm:"libmicrohttpd-debuginfo~0.9.33~2.3.amzn1", rls:"AMAZON"))) {
    report += res;
  }
