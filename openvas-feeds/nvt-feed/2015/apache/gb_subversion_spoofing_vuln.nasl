...
if(description)
{
  ...
  script_name("Apache Subversion Spoofing Vulnerability (May 2015)");
  ...
  script_tag(name:"summary", value:"Apache Subversion is prone to a spoofing vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw is due to vulnerability in the HTTP protocol used for
  communicating with a Subversion mod_dav_svn server that is triggered when handling a sequence of
  specially crafted requests.");

  script_tag(name:"impact", value:"Successful exploitation will allow an authenticated remote
  attacker to forge a svn:author value via a crafted v1 HTTP protocol request.");

  script_tag(name:"affected", value:"Subversion version 1.5.0 through 1.7.19 and 1.8.0 through
  1.8.11.");

  script_tag(name:"solution", value:"Update to version 1.7.20, 1.8.13 or later.");

  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/74259");
...
if (version_in_range(version: version, test_version: "1.5.0", test_version2: "1.7.19")) {
  fix = "1.7.20";
  VULN = TRUE;
}
