...
if (description)
{
  ...
  script_name("SolarWinds IP Address Manager (IPAM) < 4.3 Multiple SQLi Vulnerabilities");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2015 Greenbone AG");
  script_family("Web application abuses");
  script_dependencies("gb_orion_ipam_detect.nasl", "gb_solarwinds_ipam_detect_win.nasl");
  script_mandatory_keys("solarwinds/ipam/detected");

  script_tag(name:"summary", value:"SolarWinds IP Address Manager (IPAM) is prone to multiple SQL
  ...
  script_tag(name:"affected", value:"SolarWinds IPAM version 4.2 and prior.");
