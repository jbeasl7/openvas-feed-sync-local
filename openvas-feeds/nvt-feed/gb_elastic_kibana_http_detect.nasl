...
foreach dir( make_list_unique( "/", "/kibana", http_cgi_dirs( port:port ) ) ) {
  ...
  if( ( res =~ "^HTTP/1\.[01] 200" && ( egrep( string:res, pattern:"^kbn-(name|version|license-sig|xpack-sig)\s*:.+", icase:TRUE ) || "<title>Kibana</title>" >< res || res =~ "x-app-name\s*:\s*kibana" ) ) ||
      ( res2 =~ "^HTTP/1\.[01] 200" && ( egrep( string:res2, pattern:"^X-App-Name\s*:\s*kibana", icase:TRUE ) || res2 =~ "<title>Kibana [34{<]{2}" ) ) ) {
    version = "unknown";
  ...
  if( res =~ "^HTTP/1\.[01] 302" && egrep( string:res, pattern:"^kbn-(name|version|license-sig|xpack-sig)\s*:.+", icase:TRUE ) && res =~ "location\s*:.*/login\?next(Url)?=%2F.*app%2Fkibana" ) {
