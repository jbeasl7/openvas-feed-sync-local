...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.1.9.2024.6079700479899");
  ...
  script_name("Fedora: Security Advisory (FEDORA-2024-607a0047bc)");
  ...
  script_xref(name:"Advisory-ID", value:"FEDORA-2024-607a0047bc");
  script_xref(name:"URL", value:"https://bodhi.fedoraproject.org/updates/FEDORA-2024-607a0047bc");
  script_xref(name:"URL", value:"https://bugzilla.redhat.com/show_bug.cgi?id=2321662");
  ...
  script_xref(name:"URL", value:"https://github.com/python/cpython/issues/95588");

  script_tag(name:"summary", value:"The remote host is missing an update for the 'python3.9' package(s) announced via the FEDORA-2024-607a0047bc advisory.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable package version is present on the target host.");

  script_tag(name:"insight", value:"Python 3.9.21 security release.
...
- [gh-95588]([link moved to references]): Clarified the conflicting advice given in the `ast` documentation about `ast.literal_eval()` being 'safe' for use on untrusted input while at the same time warning that it can crash the process. The latter statement is true and is deemed unfixable without a large amount of work unsuitable for a bugfix. So we keep the warning and no longer claim that literal_eval is safe.");
...
if(release == "FC40") {
  ...
  if(!isnull(res = isrpmvuln(pkg:"python3.9", rpm:"python3.9~3.9.21~1.fc40", rls:"FC40"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"python3.9-debuginfo", rpm:"python3.9-debuginfo~3.9.21~1.fc40", rls:"FC40"))) {
    report += res;
  }

  if(!isnull(res = isrpmvuln(pkg:"python3.9-debugsource", rpm:"python3.9-debugsource~3.9.21~1.fc40", rls:"FC40"))) {
    report += res;
  }
