...
if( ! found ) {
  ...
  # var version='rtknas4.53/';
  loc = eregmatch( pattern:"var\s+version='([^']+)';", string:res );
  if( ! isnull( loc[1] ) ) {
    url = "/" + loc[1];
    ...
    if( 'name="Timeperiod"' >< res || 'id="deviceinfo"' >< res ) {
      url = "/nas/get/register?output=json&callback=parseNASinfo";

      req = http_get( item:url, port:port );
      res = http_keepalive_send_recv( port:port, data:req, bodyonly:TRUE );

      if( "buffalonas" >< res && "parseNASinfo" >< res ) {
        found = TRUE;
