...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.130468");
  ...
  script_xref(name:"Policy", value:"HCE Linux (Huawei Cloud EulerOS): HCE Linux Security Configuration (v1.0.0): 3. Running and Services: 3.3 SSH: 3.3.2 Configure a Proper SSH Service Authentication Mode (Requirement)");
...
non-interactive login scenarios, the public and private keys are suitable for authentication. In
high-risk scenarios, only the public and private keys can be used for authentication. If host-based
identity authentication is used, attackers can intrude the system without passwords through DNS
...
A correct authentication mode must be configured based on the service scenario requirements.");
...
Open the /etc/ssh/sshd_config file, enable IgnoreRhosts, disable HostbasedAuthentication, and
...
# grep -Ei "^PasswordAuthentication|^PubkeyAuthentication|^IgnoreRhosts|^HostbasedAuthentication" /etc/ssh/sshd_config';

expected_value = 'The output should contain a "IgnoreRhosts yes" and contain "HostbasedAuthentication no" and the output should contains a least one "PasswordAuthentication yes" or "PubkeyAuthentication yes"';
...
step_cmd = 'grep -Ei "^PasswordAuthentication|^PubkeyAuthentication|^IgnoreRhosts|^HostbasedAuthentication" /etc/ssh/sshd_config';
actual_value = ssh_cmd(socket:sock, cmd:step_cmd, return_errors:TRUE, return_linux_errors_only:TRUE);

if((strstr(actual_value, 'IgnoreRhosts yes') && strstr(actual_value, 'HostbasedAuthentication no')) && (strstr(actual_value, 'PasswordAuthentication yes') || strstr(actual_value, 'PubkeyAuthentication yes'))){
  compliant = "yes";
