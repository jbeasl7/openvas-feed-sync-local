...
if(description)
{
  ...
  script_name("Free Articles Directory RFI Vulnerability");
  ...
  script_tag(name:"summary", value:"Free Articles Directory is prone to a remote file include (RFI)
  vulnerability.");

  script_tag(name:"insight", value:"Free Articles Directory fails to sanitize user input to the
  'page' parameter in index.php. An unauthenticated attacker may be able to read arbitrary local
  files or include a file from a remote host that contains commands which will be executed by the
  vulnerable script, subject to the privileges of the web server process.");

  script_xref(name:"URL", value:"http://archives.neohapsis.com/archives/bugtraq/2006-03/0396.html");
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/17183");
...
foreach dir( make_list_unique( "/99articles", http_cgi_dirs( port:port ) ) ) {
