...
if(description)
{
  ...
  script_name("PuTTY Portable Detection (Windows SMB Login)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (C) 2018 Greenbone AG");
  script_family("Product detection");
  # nb: PuTTY dependency was added so we don't detect a registry-based installation twice
  script_dependencies("secpod_putty_smb_login_detect.nasl", "lsc_options.nasl");
  ...
  script_tag(name:"summary", value:"SMB login and WMI file search based detection of PuTTY
  Portable.");
...
fileList = powershell_wmi_file_search_version_query( file_name:"putty", file_extension:"exe" );

if( ! fileList || ! is_array( fileList ) )
  exit( 0 );

# From secpod_putty_smb_login_detect.nasl to avoid a doubled detection of a registry-based installation.
...
foreach filePath( keys( fileList ) ) {
  ...
  # to keep the install location registration the same way like in secpod_putty_smb_login_detect.nasl
  ...
  if( vers && version = eregmatch( string:vers, pattern:"^([0-9]+\.[0-9]+)" ) ) {
    ...
    register_and_report_cpe( app:"PuTTY Portable", ver:version[1], concluded:vers, base:cpe, expr:"^([0-9.]+)", insloc:location, regPort:0, regService:"smb-login" );
  }
}
