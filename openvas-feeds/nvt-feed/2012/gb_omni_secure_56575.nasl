...
if (description)
{
  ...
  script_name("Omni-Secure 5, 6, 7 Multiple File Disclosure Vulnerabilities - Active Check");
  ...
  script_tag(name:"summary", value:"Omni-Secure is prone to multiple file disclosure
  vulnerabilities.");

  script_tag(name:"vuldetect", value:"Sends a crafted HTTP GET request and checks the response.");

  script_tag(name:"impact", value:"An attacker can exploit these issues to view local files in the
  context of the web server process. This may aid in further attacks.");

  script_tag(name:"affected", value:"Omni-Secure versions 5, 6 and 7.");
...
foreach dir( make_list_unique( "/oss7", "/oss6", "/oss5", http_cgi_dirs( port:port )) ) {

  if( dir == "/" )
    dir = "";

  foreach file( files ) {
    res = http_get_cache( port:port, item:dir + "/lib" + file );
