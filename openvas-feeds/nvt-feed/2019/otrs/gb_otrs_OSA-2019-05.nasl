...
if (description)
{
  ...
  script_name("OTRS 5.x < 5.0.35, 6.x < 6.0.18, 7.x < 7.0.7 Multiple Vulnerabilities");
  ...
  - CVE-2019-9892: An attacker who is logged into OTRS as an agent user with appropriate permissions
  may try to import carefully crafted Report Statistics XML that will result in reading of arbitrary
  files on the OTRS filesystem.

  - CVE-2019-10067: An attacker who is logged into OTRS as an agent user with appropriate
  permissions may manipulate the URL to cause execution of JavaScript in the context of OTRS.");

  script_tag(name:"affected", value:"OTRS versions 5.x prior to 5.0.35, 6.x prior to 6.0.18 and
  7.x prior to 7.0.7.");
...
if (version_in_range(version: version, test_version: "5.0.0", test_version2: "5.0.34")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "5.0.35", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}

if (version_in_range(version: version, test_version: "6.0.0", test_version2: "6.0.17")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "6.0.18", install_path: location);
  security_message(port: port, data: report);
  exit(0);
}
