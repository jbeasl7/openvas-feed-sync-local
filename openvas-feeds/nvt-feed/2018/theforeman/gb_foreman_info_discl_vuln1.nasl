...
if (description)
{
  ...
  script_tag(name:"summary", value:"Foreman is vulnerable to an information disclosure
  vulnerability through organizations and locations feature.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"When a user is assigned _no_ organizations/locations, they are
  able to view all resources instead of none (mirroring an administrator's view). The user's actions
  are still limited by their assigned permissions, e.g. to control viewing, editing and deletion.");
...
if (version_is_less(version: version, test_version: "1.15.0")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "1.15.0", install_path: location);
