...
# nb: "/admin" has been added for Pi-Hole Web installations where the dir and php files are
# (currently) not gathered by the dir scanner or webmirror.nasl. This can be removed once
# both have been updated to handle this.
...
if( version || phpSessId ) {
  ...
  set_kb_item( name:"php/http/detected", value:TRUE );

  # nb: To tell http_can_host_php from http_func.inc that the service is supporting this.
