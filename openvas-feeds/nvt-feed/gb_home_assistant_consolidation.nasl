...
if( ssh_login_ports = get_kb_list( "home_assistant/ssh-login/port" ) ) {
  foreach port( ssh_login_ports ) {
    ...
    conclLoc = get_kb_item( "home_assistant/ssh-login/" + port + "/concludedLoc" );
    if( conclLoc )
      extra += '  Concluded from version/product identification location:\n    ' + conclLoc + '\n';
    else {
      conclCmd = get_kb_item( "home_assistant/ssh-login/" + port + "/concludedCmd" );
      if( conclCmd )
        extra += '  Concluded from version/product identification command:\n    ' + conclCmd + '\n';
    }
...
if( os_val && os_val =~ "Home Assistant OS" ) {
  ...
  # nb:
  # - NVD has no dedicated CPE for the OS, used same format as for App
  # - If required we could also register and report this OS here
  os_cpe = "cpe:/o:home-assistant:home-assistant";
} else {
  # Some Docker images are running on e.g.:
  # - Debian (0.86.4)
  # - Alpine (2023.x through 2025.x)
  os_name = "Linux/Unix";
