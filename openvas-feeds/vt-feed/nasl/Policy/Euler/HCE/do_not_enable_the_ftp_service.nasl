...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.134058");
  ...
  script_name("Do Not Enable the FTP Service");
  ...
  script_xref(name:"Policy", value:"Huawei Cloud HCE 2.0 Security Configuration Baseline: 1.2.8 Do Not Enable FTP (Required)(Automated)");

  script_tag(name:"summary", value:"File Transfer Protocol (FTP) is used for file transfer between
  a Linux server and other servers, desktop systems, as well as endpoints. FTP does not support
  encrypted transmission. As a result, data can be easily stolen by attackers during transmission.
  Therefore, do not install or enable the FTP service. Secure protocols such as SFTP are
  recommended.");
...
title = "Do Not Enable the FTP Service";

solution = "Disable the FTP service on a server where the vsftpd component has been installed.
# systemctl --now disable vsftpd
# systemctl --now stop vsftpd";
...
step_cmd = 'systemctl is-enabled vsftpd';
...
if(eregmatch(string: actual_value, pattern:"(No such file or directory|Permission denied|Command not found|Segmentation fault|service not found|is not running|syntax error near unexpected token|syntax error: unexpected end of file)", icase: TRUE)){
  ...
  compliant = "yes";
  comment = "The vsftpd service is disabled or not installed.";
}else{
  compliant = "no";
  comment = "The vsftpd service is enabled.";
}
