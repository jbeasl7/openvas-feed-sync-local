...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.134059");
  ...
  script_name("Do Not Enable the HTTP Service");
  ...
  script_xref(name:"Policy", value:"Huawei Cloud HCE 2.0 Security Configuration Baseline: 1.2.17 Do Not Enable HTTP (Recommended)(Automated)");
  ...
  and what responses the client receives. Request and response messages include headers in ASCII,
  and the message body often uses a MIME-like format. The HTTP server allows a client (usually a
  browser) to request web content such as webpages, images, and documents through HTTP and
  transfers the content to the client.");
...
step_cmd = 'systemctl is-enabled httpd';
...
if(eregmatch(string: actual_value, pattern:"(No such file or directory|Permission denied|Command not found|Segmentation fault|service not found|is not running|syntax error near unexpected token|syntax error: unexpected end of file)", icase: TRUE)){
  ...
  compliant = "yes";
  comment = "The httpd service is disabled or not installed.";
}else{
  compliant = "no";
  comment = "The httpd service is enabled.";
}
