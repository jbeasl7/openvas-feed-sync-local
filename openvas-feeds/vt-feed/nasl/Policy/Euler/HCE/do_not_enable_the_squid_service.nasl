...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.134060");
  ...
  script_name("Do Not Enable the Squid Service");
  ...
  script_xref(name:"Policy", value:"Huawei Cloud HCE 2.0 Security Configuration Baseline: 1.2.6 Do Not Enable Squid (Required)(Automated)");

  script_tag(name:"summary", value:"Squid is a web cache proxy service that supports protocols such
  as HTTP, HTTPS, and FTP. Unless the system is intentionally used as a proxy server, disable the
  Squid service when the web proxy is not required. This reduces the system's attack surface.");
...
title = "Do Not Enable the Squid Service";

solution = "Disable the Squid service on a server where the Squid component has been installed.
# systemctl --now disable squid
# systemctl --now stop squid";
...
step_cmd = 'systemctl is-enabled squid';
...
if(eregmatch(string: actual_value, pattern:"(No such file or directory|Permission denied|Command not found|Segmentation fault|service not found|is not running|syntax error near unexpected token|syntax error: unexpected end of file)", icase: TRUE)){
  ...
  compliant = "yes";
  comment = "The Squid service is disabled or not installed.";
}else{
  compliant = "no";
  comment = "The Squid service is enabled.";
}
