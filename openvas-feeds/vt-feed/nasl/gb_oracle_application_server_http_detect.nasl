...
if(description)
{
  ...
  script_name("Oracle Application / HTTP Server Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of the Oracle Application Server (AS) or
  Oracle HTTP Server.");
...
if( buf =~ "Server\s*:.*Oracle[ -]Application[ -]Server" ) {
  set_kb_item( name: "oracle/application_server/detected", value: TRUE );
  set_kb_item( name: "oracle/application_server/http/detected", value: TRUE );
...
if( buf =~ "Server\s*:.*Oracle[ -]HTTP[ -]Server" ) {
  set_kb_item( name: "oracle/http_server/detected", value: TRUE );
  set_kb_item( name: "oracle/http_server/http/detected", value: TRUE );
