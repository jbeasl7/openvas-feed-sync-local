...
if(description)
{
  ...
  script_name("Horde IMP Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of Horde IMP (Internet Messaging
  Program).");

  script_xref(name:"URL", value:"https://www.horde.org/apps/imp/");
...
foreach dir( make_list_unique( "/webmail", "/horde", "/horde/imp", "/email", "/imp", "/mail", http_cgi_dirs( port:port ) ) ) {
  ...
  if( res && res =~ "^HTTP/1\.[01] 200" &&
      ( "<!-- IMP: Copyright" >< res ||
        ...
        res =~ "IMP: https?://(www\.)?horde.org/imp/"
      ) ) {
    ...
    set_kb_item( name:"horde/imp/http/detected", value:TRUE );
    ...
    cpe = build_cpe( value:version, exp:"^([0-9.]+)", base:"cpe:/a:horde:imp:" );
    if( ! cpe )
      cpe = "cpe:/a:horde:imp";

    register_product( cpe:cpe, location:install, port:port, service:"www" );

    log_message( data:build_detection_report( app: "Horde IMP", version:version, install:install, cpe:cpe,
