# SPDX-FileCopyrightText: 2017 Greenbone AG
# Some text descriptions might be excerpted from (a) referenced
# source(s), and are Copyright (C) by the respective right holder(s).
#
# SPDX-License-Identifier: GPL-2.0-only

CPE = "cpe:/a:zohocorp:manageengine_firewall_analyzer";

if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.811534");
  script_version("2026-04-22T06:16:44+0000");
  script_tag(name:"last_modification", value:"2026-04-22 06:16:44 +0000 (Wed, 22 Apr 2026)");
  script_tag(name:"creation_date", value:"2017-07-19 14:54:04 +0530 (Wed, 19 Jul 2017)");
  script_tag(name:"cvss_base", value:"5.0");
  script_tag(name:"cvss_base_vector", value:"AV:N/AC:L/Au:N/C:P/I:N/A:N");
  script_tag(name:"severity_vector", value:"CVSS:3.0/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:N/A:N");
  script_tag(name:"severity_origin", value:"NVD");
  script_tag(name:"severity_date", value:"2017-06-30 17:58:00 +0000 (Fri, 30 Jun 2017)");

  script_cve_id("CVE-2015-7780", "CVE-2015-7781");

  script_tag(name:"qod_type", value:"remote_banner");

  script_tag(name:"solution_type", value:"VendorFix");

  script_name("ManageEngine Firewall Analyzer < 8.0 Multiple Vulnerabilities");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2017 Greenbone AG");
  script_family("Web application abuses");
  script_dependencies("gb_manageengine_firewall_analyzer_consolidation.nasl");
  script_mandatory_keys("manageengine/firewall_analyzer/detected");

  script_tag(name:"summary", value:"ManageEngine Firewall Analyzer is prone to access bypass and
  directory traversal vulnerabilities.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Multiple flaws exist due to:

  - Access permissions are not restricted.

  - A directory traversal error.");

  script_tag(name:"impact", value:"Successfully exploitation will allow remote attackers to obtain
  arbitrary files on the server and bypass security restrictions and perform unauthorized actions.
  This may aid in launching further attacks.");

  script_tag(name:"affected", value:"ManageEngine Firewall Analyzer prior to version 8.0.");

  script_tag(name:"solution", value:"Update to version 8.0 or later.");

  script_xref(name:"URL", value:"http://jvn.jp/en/jp/JVN12991684/index.html");
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/78211");
  script_xref(name:"URL", value:"http://www.securityfocus.com/bid/78213");
  script_xref(name:"URL", value:"http://jvn.jp/en/jp/JVN21968837/index.html");

  exit(0);
}

include("host_details.inc");
include("version_func.inc");

if (!version = get_app_version(cpe: CPE, nofork: TRUE))
  exit(0);

if (version_is_less(version: version, test_version: "8.0")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "8.0");
  security_message(port: 0, data: report);
  exit(0);
}

exit(99);
