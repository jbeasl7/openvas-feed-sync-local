...
if(description)
{
  ...
  script_name("ManageEngine Applications Manager Detection (HTTP)");

  script_tag(name:"summary", value:"HTTP based detection of ManageEngine Applications Manager.");
...
foreach dir( make_list_unique( "/", "/manageengine", http_cgi_dirs( port:port ) ) ) {
  ...
  if( "manageengine" >< res && '<title>Applications Manager Login Screen</title>' >< res ) {
    ...
    set_kb_item(name: "manageengine/applications_manager/detected", value: TRUE);
    set_kb_item(name: "manageengine/applications_manager/http/detected", value: TRUE);
